...
#     item_price = None
#     for se in se_list:
#         item_line_price = frappe.get_value("Sales Invoice Item", {"parent":se.get("name"), "item_code":args.get("item_code")},"rate")

#         if item_line_price and item_line_price > 0:
#             item_price = item_line_price
#             break
#     if item_price != None:
#         return item_price
#     else:
...
from __future__ import unicode_literals
from console import console #custom logic
...
def get_price_list_rate(args, item_doc, out=None):
	...
	if meta.get_field("currency") or args.get('currency'):
		...
		if price_list_rate is None:
			...
			return out
		if get_rate(args) > 0:
			console("enter in get rate if").log()
			out.price_list_rate = flt(price_list_rate) * flt(args.plc_conversion_rate) \
			/ flt(args.conversion_rate)
			out.previous_rate = get_rate(args)
			console("out.previous_rate").log()
			console(out.previous_rate).log()
			#out.price_list_rate = get_rate(args)
		else:
			out.price_list_rate = flt(price_list_rate) * flt(args.plc_conversion_rate) \
...
#Replica of get_rate method
@frappe.whitelist()
def get_quo_rate(customer, item_code):
    q_list = frappe.db.get_all("Quotation", {"customer_name": customer, "docstatus": 1})
    console("se_list").log()
    console(q_list).log()
    item_price = None
    for se in q_list:
        item_line_price = frappe.get_value("Quotation Item", {"parent": se.get("name"), "item_code": item_code}, "rate")
        console("item_line_price").log()
        console(item_line_price).log()
